...
def load_gui(self):
    ...
    #add the listbox and the scrollbar
    ...
    self.btn_add = tk.Button(self.master,width=12,height=2,text='Add',command=lambda: studenttrack_func.addToList(self))
    self.btn_add.grid(row=10,column=0,padx=(25,0),pady=(45,10),sticky=W)
    self.btn_delete = tk.Button(self.master,width=12,height=2,text='Delete',command=lambda: studenttrack_func.onDelete(self))
    self.btn_delete.grid(row=10,column=1,padx=(15,0),pady=(45,10),sticky=W)

    studenttrack_func.create_db(self)
    studenttrack_func.onRefresh(self)
